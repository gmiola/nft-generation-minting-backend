...
def setup(data):
    ...
    ###IMPORTANT: the goal of 'data' is to avoid readng parameters from the paths.txt file, however the paths.txt file will be read anyway,
    #allowing the flow of the old code to work

    basePath = '../uploads/'+data["address"]+'/'
    ...
    try:
        my_file.readline()                                    #try to read the path where i'll store my NFTs
        destination = basePath + 'Results/results'
    ...
    try:
        my_file.readline()                                  #the second value that you read is the number of layers in the NFT
        nLayers = data["numLayers"]
    except:
        print("the second line of pahts.txt is not a number...")            #error message in case the second line of the file is not a number
    try:
        my_file.readline()                                  #the third number is the number of NFTs the user wants to create
        num_NFT = data["numNFTtoGenerate"]
    except:
...
def step2(my_file, nLayers, nElements, data):
    ...
    print("suo nelements: ",nElements)
    nElements = data["cardinalities"]
    print("mio nelements: ",nElements)

    layers = data["layers"] #layer names
...
def randomness(nLayers, nElements,randoms,fields,d,collectible_metadata, data):   #This function takes care of the random generation of the image's 
    ...
    ###MIOLA### each layer's image will have a certain probability to be selected
    # the probability factor can be provided by the user, or if none is provided they
    # will have equal probability
   
    print("nLayers: ",nLayers)
    print("nelements = cardinalities?: ",nElements)                                         
    for k in range(1,nLayers):                                              #For all the layers of our image and for each element in the layer,
        #for j in range(0,nElements[k]):                                     #we will fill the temp_list and the temp_weights as described earlier,
        ...
        temp_weight = data["weights"][k].copy()
        temp_weight.pop(0)
        print("temp_l: ",temp_list)
        print("temp_w: ", temp_weight)
...
def NFT_generator(paths,randoms,pattern,num_NFT,metadata_save,nLayers,nElements,Layer,log,destination,iterations,count,d,fields,data):
    ...
    while len(pattern) < num_NFT :                                          #this is the whole cycle that generates our NFTs
        ...
        randoms.append(0)                                                   #the first value of the randoms is 0 cause the first layer has only
                                                                            #the base image.
        randomness(nLayers, nElements,randoms,fields,d,collectible_metadata,data)#i calle the randomness function create earlier to generate the new
                                                                            #NFT. 
...
def main():
    #local variables to avoid the use of paths.txt. Passed in as a json argument
    print("Python got data: ", sys.argv[1], type(sys.argv[1]))
    data = json.loads(sys.argv[1])
    print("To json: ", data, type(data))
    address = data["address"]
    numLayers = data["numLayers"]
    numNFTtoGenerate = data["numNFTtoGenerate"]
    layers = data["layers"]
    cardinalities = data["cardinalities"]
    print("Data: ", data, type(data))
    print("Address: ", address, type(address))
    print("nLayers: ", numLayers, type(numLayers))
    ...
    NFT_generator(paths,randoms,pattern,num_NFT,metadata_save,nLayers,nElements,Layer,log,destination,iterations,count,d,fields,data)#i generate all
                                                                            #the images.
